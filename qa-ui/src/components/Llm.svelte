<script>
  ...
  questionsAndAnswers = [...questionsAndAnswers, { id: tempQuestionId, question: question, llmAnswers: [], humanAnswers: [] }];
  ...
  const index = questionsAndAnswers.findIndex(qna => qna.id === tempQuestionId);
  if (index !== -1) {
...
const fetchQuestionsAndAnswers = async () => {
  const response = await fetch(`/api/getQuestionsAndAnswers?courseId=${$courseId}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  let updatedQuestionsAndAnswers = jsonData.map(async (qna) => {
    ...
    return { ...qna, llmAnswers, humanAnswers, votes };
  });
